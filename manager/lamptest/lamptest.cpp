...
void LampTest::storePhysicalLEDsStates()
{
    ...
    for (const auto& path : physicalLEDPaths)
    {
        ...
        Layout::Action action = getActionFromString(state);
        if (action != Layout::Action::Off)
        {
            Layout::LedAction ledAction{name, action, dutyOn, period,
                                        Layout::Action::On};
...
void LampTest::start()
{
    if (isLampTestRunning)
    {
        ...
        // Notify PHYP to reset the timer
        doHostLampTest(true);
